...
/**
 * DeleteCardConfirmationDialog Component
 * 
 * Shows a confirmation dialog before deleting a card
 * 
 * Props:
 * - card: UserCard object to delete
 * - benefitCount: number - count of benefits that will be deleted
...
export function DeleteCardConfirmationDialog({
  card,
  benefitCount,
  isOpen,
  onClose,
  onConfirm,
}: DeleteCardConfirmationDialogProps) {
    ...
    <DialogPrimitive.Root open={isOpen} onOpenChange={(open) => {
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className="fixed left-[50%] top-[50%] z-50 w-full max-w-[calc(100%-2rem)] sm:max-w-sm translate-x-[-50%] translate-y-[-50%] rounded-lg shadow-lg data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] duration-200 p-6 mx-4"
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-md p-3 text-sm text-red-900 dark:text-red-100">
              This will delete the card <strong>AND all {benefitCount} benefit{benefitCount !== 1 ? 's' : ''}</strong>. This action cannot be undone.
            </div>
